...
def Agente0(matriz, matrizCaminho, lista, listaAux, v, visitados=None, tempo_por_casa=None, tempo_casas_e_ruas=None,
    ...
    for adj in lista[v]:
        if (v, adj) not in visitados and (adj, v) not in visitados:
            if ruaSemSaida(listaAux, v, visitados) == v:
                ...
                Agente0(matriz, matrizCaminho, lista, listaAux, adj, visitados, tempo_por_casa,
...
def verificaSeAcabou(matrizCaminho, listaAux, visitados, v, V, conferir, distancia, tempo_casas_e_ruas, qtd_ruas_repetidas):
    if all(not lista for lista in listaAux.values()) and conferir[0] == 0:
